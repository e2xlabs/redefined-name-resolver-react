...
import { DropDownReverseProps } from "../../types";
...
const DropDown = (props: DropDownReverseProps) => {
  ...
  const onCopyClick = (event: any, domain: string) => {
    copyText(domain);
  ...
  return active ? (
    <DropDownWrapper ref={ref} data-testid="dropdown">
      ...
      {!loading && content.length > 0 ? <UnorderedList>
        {content.map((item: ReverseAccount, key) => (
          <ListItem key={key}>
      ...
      {!loading && !error && content.length == 0 ? <StyledNotFoundMessage>No domains found</StyledNotFoundMessage> : null}
